...
def transform_data(df_books, df_ratings):
    ...
    try:
        ...
        # review_score: eliminar nulos (escala 1-5; un 0 distorsionaría el modelo de IA)
        df_merged = df_merged.dropna(subset=['review_score'])
        df_merged['categories'] = df_merged['categories'].fillna('Unknown')
        df_merged['authors'] = df_merged['authors'].fillna('Unknown')
        # Filtrar timestamps inválidos (Unix 0 = 1969-12-31)
        if 'review_date' in df_merged.columns:
            df_merged = df_merged[df_merged['review_date'].dt.year > 1970]
